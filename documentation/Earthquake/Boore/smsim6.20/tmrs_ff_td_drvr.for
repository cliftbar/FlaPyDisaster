* ----------------- BEGIN TMRS_FF_TD_Drvr ---------------------------
      Program TMRS_FF_TD_Drvr
...
!        01/17/11 - dur_95_05, dur_95_05gm added to gm_td subroutine output
...
!                   to the summary file in write_params, after the call to get_params
     ...
     :                   time_begin*10, time_end*10
      ...
      real psvsim(1), psvsim_std(1), 
     :     psvsimgm(1), psvsimgm_std(1), 
     :     per(1), 
     :     acc_save(1), vel_save(1), dis_save(1)

!      logical tdflag, fparam_exist, new_mr ! new_mr is declared in smsim.fi
...
333   continue
     ...
     :    '("Enter" = tmrs_ff_td_drvr.ctl): '
        read(*, '(a)') f_ctl
        call trim_c(f_ctl,nc_f_ctl)
        if (f_ctl(1:4) .eq. '    ') 
     :                         f_ctl = 'tmrs_ff_td_drvr.ctl'
        call trim_c(f_ctl, nc_f_ctl)
...
888   continue
     ...
     :                      siteloc1_in, siteloc2_in
      ...
      print *,' damp, perosc, amag, stress = ', 
     :          damp, perosc, amag, stress_in
...
* save psa:
        ...
        sd = (perosc/twopi)*psv
        psvgm= psvsimgm(1)
        psagm = (twopi/perosc)*psvgm     ! convert psv to psa 
        psagmg = psagm/981.0
        sdgm = (perosc/twopi)*psvgm
        freqosc = 1.0/perosc
      end if
